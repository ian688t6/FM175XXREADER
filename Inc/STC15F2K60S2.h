...
sfr WDT_CONTR=0xC1;	//不可用&/|操作  复位后为xx00 0000
                    //WDT_CONTR^7=1　看门狗有溢出,软件清零
                    //WDT_CONTR^5=1　启动看门狗;　 
					//WDT_CONTR^4=1　看门狗将重新计数,硬件清此位;　
					//WDT_CONTR^3=0	 看门狗"空闲模式"不计数,WDT_CONTR^3=1	 看门狗"空闲模式"计数
					//溢出时间=(N*2^(WDT_CONTR^2　WDT_CONTR^1　WDT_CONTR^0　+1)*32768/Fcsc)
					//N为CPU工作在几T模式,Fclock为晶振频率
//串行口特殊功能寄存器
...
sfr ISP_DATA=0xC2;	//isp/iap数据寄存器   复位后为0xff
sfr ISP_ADDRH=0xC3;	//isp/iap操作高8位地址 复位后为0x00
sfr ISP_ADDRL=0xC4;	//isp/iap操作低8位地址 复位后为0x00 
sfr ISP_CMD=0xC5;	//复位后为xxxx xx00
                    //isp/iap命令模式寄存器  0待机,无isp/iap操作;1字节读操作;2字节写(该字节必须为空0xFF);3扇区擦除
sfr ISP_TRIG=0xC6;	//xxxx xxxx
                    //isp/iap操作命令触发寄存器,在isp_contr^7=1时,对isp_trig依次写入0x5A,0xA5
sfr ISP_CONTR=0xC7;	//isp_iap控制寄存器　 复位后为0000 1000
					//　isp_contr^7　isp_iap功能允许位,0:禁止isp_iap编程改变Flash,1:允许isp_iap编程改变Flash
					//　isp_contr^6　0:从用户AP程序启动,1:从ISP程序区启动
					//　isp_contr^5　0:不操作,1:产生软件系统复位,硬件自动清零
					//　isp_contr^4　
					//　isp_contr^3　
					//　isp_contr^2　isp_contr^1　isp_contr^0  CPU等待时间　<40M时000,　<20M时001,　<10M时, 010,　<5M时 011

#define isp_trig1 0x5a
#define isp_trig2 0xa5
