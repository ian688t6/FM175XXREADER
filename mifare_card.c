 ...
 unsigned char Mifare_Auth(unsigned char mode,unsigned char sector,unsigned char *mifare_key,unsigned char *card_uid)
{
	...
	result =PcdComCmd(PCD_AUTHENT,send_buff,12,rece_buff,&rece_bitlen);//Authent认证
	if (result==0)
	{
		if(ReadRawRC(Status2Reg)&0x08)//判断加密标志位，确认认证结果
			return 0;
		else
			return 1;
	}
...
//	Pcd_SetTimer(5);
	...
	if ((result!=0)|((rece_buff[0]&0x0F)!=0x0A))	//如果未接收到0x0A，表示无ACK
	return(1);
